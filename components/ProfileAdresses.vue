...
const submitHandler = handleSubmit(async (item) => {
...
const schema = object({
    addresses: array().of(object().shape({
        address: object({
            ...
            house_number: number("Значение должно быть числом").required("Обязательное поле"),
            apartment_number: number("Значение должно быть числом").required("Обязательное поле"),
        })
    }))
});
...
const submitHandler = handleSubmit(async (item) => {
    ...
    if (status.value === 'success') {
        ...
        setErrors(formatErrors(errors, 'addresses'))
    }
