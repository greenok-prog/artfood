    ...
    <div class="review">
        <div class="review__inner">
            <div class="review__image">
                <img v-if="review.images" :src="review.images[0].image"
                    alt="">
            </div>
            <div class="review__content">
                <h4 class="review__content-name">{{ review.name }}</h4>
                <div class="review__content-rating">
                    <span>Поставьте оценку</span>

                    <nuxt-rating :read-only="false"
                        :ratingValue="review.count_star"
                        rating-size="24px"
        ...
        <div class="review__action">
            <VBtn @click="sendReview" class="review__button">Изменить
                отзыв</VBtn>
        </div>
    </div>
</template>
...
const sendReview = async () => {
    ...
    const { data, status } = await useAuthFetch(`/api/change-review/${props.review.id_review}`, {
        method: 'put',
        body: {
            count_stars: rating.value,
            review_text: text.value,
            product: props.review.id,
